...
class Desk(object):
    def __init__(self,rank='2',mode="RankFrozen",port='23334'):
        ...
        self.teamALevel='2'
        self.teamBLevel='2'
        self.players=[Player(Position.NORTH),Player(Position.SOUTH),Player(Position.WEST),Player(Position.EAST)]
        self.player_num=0
        self.state=None
    ...
    def begin(self):
        ...
        #和client连接
        asyncio.get_event_loop().run_until_complete(websockets.serve(self.playgame, 'localhost', 10000))
        asyncio.get_event_loop().run_forever()
    
    async def playgame(self, websocket, path):
        ...
        if self.player_num >= 4:    #连接的client达到4个
            while(True):
                if num == 0:
                    ...
                    #升级什么的
                if shuffle == True:
                    self.send_begin_deck(websocket, num)     #向当前玩家发送初始手牌
                    self.runPlay(websocket, num)
                    self.setLevel()
                    self.tribute()
                if self.total_game == 0:  #达到指定局数
                    break
